...
const calculateVulnerabilityRate = (scanType, results) => {
  ...
  switch (scanType.toLowerCase()) {
    case 'port scanner':
      return results.openPorts?.length > 0 ? (results.openPorts.length / 10) * 100 : 0;
    case 'clickjacking detection':
...
const formatScanResults = (scanType, results) => {
  ...
  switch (scanType.toLowerCase()) {
    case 'port scanner':
      return results.openPorts?.length > 0 
        ? `Open ports found: ${results.openPorts.join(', ')}`
        : 'No open ports found';
    case 'clickjacking detection':
    ...
    const fetchScanData = async () => {
      try {
        ...
        const transformedData = latestScans.map(scan => ({
          attack: scan.type.charAt(0).toUpperCase() + scan.type.slice(1),
